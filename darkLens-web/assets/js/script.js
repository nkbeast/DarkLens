...
$(document).ready(function(){
    $.post("list_templates.php",function(data){
        ...
        for(let i = 0; i < get_json.length;){
            ...
            $("#links").append(
                '<div class="link-row" style="display:flex; align-items:center; justify-content:center; margin-top:10px;">' +
                  '<p class="link-text" style="color:#0f0; background:#111; padding:10px; margin:5px; border-radius:4px; width:60%; overflow-wrap:anywhere;">' +
                    "http://" + location.host + "/templates/" + get_json[i] + "/index.html" +
                  '</p>' +
                  '<button class="copy-btn" style="margin-left:10px; padding:10px 15px; background:#0f0; color:#000; font-weight:bold; border:none; border-radius:4px; cursor:pointer;" onclick="copyToClipboard(this)">Copy</button>' +
                '</div>'
              )
            i++
